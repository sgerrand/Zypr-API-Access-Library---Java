  ...
  extends BackgroundProcess
{
  ...
  public FriendListUpdater(int interval)
  {
    super(interval);
  }
    ...
    while (_running)
      {
        try
          {
            Session.getInstance().setFriendList(API.getInstance().getSocial().friendList(), Session.getInstance().isPartialAPICall(APIVerbs.SOCIAL_FRIEND_LIST));
            waitForInterval();
          }
